...
function burn {
  ...
  until False
  {
      ...
      {
          print "End burn, remain dv " + round(node2:deltav:mag,1) + "m/s, vdot: " + round(vdot(dv0, node2:deltav),1).
...
//          lock throttle to 0.
      ...
      //print "remain dv=" + node2:deltav:mag.
...
function burn_sequence {
  print "v2".
  local node2 to NEXTNODE.
  local lock max_acc to ship:maxthrust / ship:mass.
  local burn_duration to node2:deltav:mag / max_acc.

  print "Node in: " + round(node2:eta) + ", DeltaV: " + round(node2:deltav:mag).
  print "Estimated burn duration: " + round(burn_duration) + "s".
  local lock burn_start to node2:eta - burn_duration / 2.
  set_course(node2).
